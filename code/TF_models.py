...
class AtlasNet:
  ...
  def __init__(self, params):
    ...
    with g_atlas.as_default():
        with tf.device('/gpu:0'):
            ...
            self.ip4 = tf.add(tf.matmul(self.o2_flat, self.w4), self.b4, name=self.network_name + '_' + layer_name + '_ips')
            self.o4 = tf.nn.relu(self.ip4, name=self.network_name + '_' + layer_name + '_activations')

            # Layer 5 (Fully connected, parameter output)
            layer_name = 'fc5'
            ...
            self.cost_z = tf.reduce_sum(tf.pow(tf.subtract(self.tformed, fixed), 2), name='RMS')
  ...
  def train(self, bat_In, param):
    feed_dict = {self.In: bat_In, self.Ideal:param, self.keep_prob:.60}
    _, cnt, cost, out, cost_t, tformed = self.sess.run([self.rmsprop_cc, self.global_step, self.cost_sup, self.xytheta, self.cost_cc, self.tformed], feed_dict=feed_dict)
    return cnt, cost/bat_In.shape[0], out, cost_t, tformed
  ...
  def run_batch(self,batch):
    ...
    tformed,xytheta,cost_cc = self.sess.run([self.tformed,self.xytheta,self.cost_cc], feed_dict=feed_dict)
...
class ElasticNet:
  ...
  def train(self, bat_In):
    feed_dict = {self.In: bat_In,
                 self.keep_prob:.60,
                 self.weight_se:[1.0],
                 self.weight_E_det_j:[10.0],
                 self.weight_E_div:[15.0],
                 self.weight_E_curl:[10.0]}
    _, cnt, cost, cost_cc, E_det_j, E_div, E_curl = self.sess.run([self.rmsprop, self.global_step, self.cost, self.cost_cc, self.E_det_j, self.E_div, self.E_curl], feed_dict=feed_dict)
...
class ElasticTransformer(object):
  ...
  def transform(self, inp, theta, forward=True, batch_size = 32, **kwargs):
    ...
    with tf.variable_scope(self.name):
      ...
      #batch,1,y,x
      E_det_j,E_div,E_curl = warp_E(tf.reshape(self.dxy[:,0,:,:],[-1, self.grid_size, self.grid_size, 1]), tf.reshape(self.dxy[:,1,:,:],[-1, self.grid_size, self.grid_size, 1]))
  ...
  def _transform(self, inp, theta, num_control_points, pixel_grid, num_pixels, pixel_distances, L_inv, name, batch_size,
                 forward=True):
    with tf.variable_scope(name):
